...
void TT::clear() { table.assign(table.size(), Entry{}); }
...
const TT::Entry *TT::probe(uint64_t key) {
  if (table.empty()) return nullptr;

  const Entry &entry = table[key & mask];
  return (entry.key == key && entry.bound != NONE) ? &entry : nullptr;
}

void TT::store(uint64_t key, const Move &move, int score, int depth, Bound bound) {
  if (table.empty()) return;

  Entry &entry = table[key & mask];

  // Same position searched shallower: keep the deeper result.
